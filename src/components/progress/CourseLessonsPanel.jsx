...
import {
  ...
  ExternalLink, Download, Video, FileText, File, Image,
  BookOpen, Lock, HelpCircle, Trophy, AlertCircle, GraduationCap
} from 'lucide-react'
import QuizTracker from '../quiz/QuizTracker'
...
// ── FINAL QUIZ SECTION (student) ──────────────────
function FinalQuizSection({ courseId, moduleStatuses }) {
  const qc = useQueryClient()
  const [quizOpen, setQuizOpen] = useState(false)

  // Read final quiz state from any moduleStatus entry (all have same finalQuiz fields)
  const anyStatus = moduleStatuses[0]
  const allModulesComplete = anyStatus?.allModulesComplete ?? false
  const finalQuizExists    = anyStatus?.finalQuizExists    ?? false
  const finalQuizPassed    = anyStatus?.finalQuizPassed    ?? false
  const finalQuizBestScore = anyStatus?.finalQuizBestScore ?? null
  const finalQuizPassing   = anyStatus?.finalQuizPassingScore ?? null
  const finalQuizId        = anyStatus?.finalQuizId        ?? null

  // Fetch quiz questions (without correct answers) when section unlocked
  const { data: quiz } = useQuery({
    queryKey: ['final-quiz-student', courseId],
    queryFn: () => quizApi.getFinalForStudent(courseId).then(r => r.data),
    enabled: !!courseId && allModulesComplete && finalQuizExists,
    retry: false,
  })

  if (!finalQuizExists) return null
  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-xs font-bold text-amber-600 uppercase tracking-widest">Final Exam</span>
        <div className="flex-1 h-px bg-gradient-to-r from-transparent via-amber-200 to-transparent" />
      </div>
      ...
      <div className={`rounded-2xl border-2 px-5 py-4 flex items-center gap-4 ${
        ...
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0 ${
          finalQuizPassed ? 'bg-emerald-100' : allModulesComplete ? 'bg-amber-100' : 'bg-gray-100'
        }`}>
          {finalQuizPassed
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {finalQuizPassed
              ? `✅ Passed · ${finalQuizBestScore}% (need ${finalQuizPassing}%)`
              : allModulesComplete
                ? finalQuizBestScore != null
                  ? `Best score: ${finalQuizBestScore}% · Need ${finalQuizPassing}% to pass`
                  : `Pass mark: ${finalQuizPassing}% · Unlocked!`
                : 'Complete all module quizzes to unlock this exam'
            }
          </p>
        </div>
        ...
        ) : allModulesComplete && quiz ? (
          <button
            onClick={() => setQuizOpen(true)}
            className="flex-shrink-0 text-sm font-semibold bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-xl transition-colors"
          >
            {finalQuizBestScore != null ? 'Retake Exam' : 'Start Exam'}
          </button>
        ) : (
      ...
      {quiz && (
        <QuizTracker
          ...
          onPassed={() => {
            setQuizOpen(false)
            qc.invalidateQueries({ queryKey: ['module-statuses', courseId] })
            qc.invalidateQueries({ queryKey: ['final-quiz-student', courseId] })
            qc.invalidateQueries({ queryKey: ['my-certificates'] })
          }}
        />
      )}
...
export default function CourseLessonsPanel({ courseId, progressMap }) {
  ...
  return (
    ...
    >
      ...
      {/* Final exam — appears after all modules */}
      <FinalQuizSection courseId={courseId} moduleStatuses={moduleStatuses} />
    </div>
  )
}
