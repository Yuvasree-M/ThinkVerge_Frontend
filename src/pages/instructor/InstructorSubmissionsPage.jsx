// pages/instructor/InstructorSubmissionsPage.jsx
...
import {
  FileText, Star, ChevronDown, ChevronRight,
  Download, Eye, Clock
} from 'lucide-react'
...
export default function InstructorSubmissionsPage() {
  ...
  const statusBadge = (status) => ({
    LATE:      <span className="badge-red   text-xs">Late</span>,
    EARLY:     <span className="badge-green text-xs">Early</span>,
    SUBMITTED: <span className="badge-blue  text-xs">Submitted</span>,
  }[status] || null)
        ...
        {courses.map(c => (
          <div key={c.id}>
            <button
              ...
              onClick={() => {
                setSelectedCourse(c.id === selectedCourse ? null : c.id)
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg text-xs transition-all ${
                      selectedAssignment === a.id
                        ...
                        : 'text-navy-600 hover:bg-royal-50'
                    }`}
                    onClick={() => setSelectedAssignment(a.id)}
                  >
                    <span className="flex items-center gap-1.5">
                      {a.pdfUrl && <FileText size={10} />}
                      {a.title}
                    </span>
                  </button>
                ))}
      ...
      <div className="lg:col-span-2">
        {!selectedAssignment ? (
          <EmptyState
            ...
            description="Choose a course and assignment from the left to view submissions."
          />
        ) : ls ? (
          <PageSpinner />
              ...
              <div key={sub.id} className="card hover:shadow-card-hover transition-shadow">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-navy-800 text-sm">{sub.studentName || '—'}</p>
                      <div className="flex items-center gap-2 mt-0.5">
                        {statusBadge(sub.status)}
                        {sub.submittedAt && (
                          <span className="text-xs text-slate-lms flex items-center gap-1">
                            <Clock size={10} />
                ...
                {sub.content && (
                  <p className="mt-3 text-sm text-navy-600 bg-surface rounded-lg p-3 line-clamp-2">
                    {sub.content}
                  </p>
                )}

                {/* ✅ PDF submission — inline preview + view + download */}
                {sub.fileUrl && (
                  <div className="mt-3 border border-royal-100 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-3 py-2 bg-royal-50 border-b border-royal-100">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-medium text-navy-700">Submitted PDF</span>
                      </div>
                      <div className="flex items-center gap-1">
                        ...
                        {/* ✅ explicit filename so download saves as .pdf */}
                        <a
                          href={sub.fileUrl}
                          download={`${sub.studentName || 'student'}_submission.pdf`}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    {/* ✅ Google Docs viewer for inline PDF display */}
                    <iframe
                      src={pdfViewerUrl(sub.fileUrl)}
                      title={`${sub.studentName} submission`}
                      className="w-full"
                      style={{ height: 240 }}
                    />
                  </div>
