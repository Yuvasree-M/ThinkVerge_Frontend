...
import useModal from '../../hooks/useModal'
import CourseLessonsPanel from '../../components/progress/CourseLessonsPanel'
import CourseFeedbackModal from '../../components/feedback/CourseFeedbackModal'

import { BookOpen, Clock, XCircle, PlayCircle, ChevronRight, Award, MessageSquare } from 'lucide-react'
    ...
    <>
      <article className="group bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 overflow-hidden flex flex-col">
        ...
        <div className="flex flex-col flex-1 p-4 gap-4">
          ...
          {/* CTA BUTTONS */}
          <div className={`mt-auto flex gap-2 ${isComplete ? 'flex-col' : ''}`}>
            <button
              onClick={onOpen}
              className={`flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 ${
                isComplete
                  ? 'w-full bg-emerald-50 text-emerald-700 hover:bg-emerald-100'
                  : 'flex-1 bg-indigo-600 text-white hover:bg-indigo-700 active:scale-[0.98]'
              }`}
            >
              {isComplete ? (
                <>
            ...
            {/* ✅ Feedback button — only shown on completed courses */}
            {isComplete && (
              <button
                onClick={() => setFeedbackOpen(true)}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium bg-indigo-50 text-indigo-600 hover:bg-indigo-100 active:scale-[0.98] transition-all duration-150"
              >
